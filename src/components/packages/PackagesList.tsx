...
export default function PackagesList({ packages, locale, userId }: Props) {
  ...
  const PackageCard = ({ pkg }: { pkg: Package }) => {
    ...
    return (
      <div className={`bg-white rounded-2xl border shadow-sm p-6 flex flex-col ${isPremium ? 'border-primary ring-2 ring-primary/20' : 'border-border'}`}>
        {isPremium && (
          <span className="self-start text-xs font-bold uppercase tracking-widest bg-primary text-primary-foreground px-2 py-0.5 rounded-full mb-3">
            Premium
          </span>
        )}
        <h3 className="font-bold text-primary text-lg leading-tight mb-1">{name}</h3>
        <p className="text-3xl font-extrabold text-primary mt-2 mb-1">
        ...
        {pkg.allowed_class_types && pkg.allowed_class_types.length > 0 && (
          <div className="mb-4">
            <p className="text-xs font-medium text-primary mb-1">{t('includes')}:</p>
            <div className="flex flex-wrap gap-1">
              {pkg.allowed_class_types.map((type) => (
                <span key={type} className="text-xs bg-secondary text-primary px-2 py-0.5 rounded-full">
  ...
  const Section = ({ title, pkgs }: { title: string; pkgs: Package[] }) => {
    ...
    return (
      <div className="mb-12">
        <h2 className="text-xl font-bold text-primary mb-5">{title}</h2>
